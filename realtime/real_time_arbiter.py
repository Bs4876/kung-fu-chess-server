...
from realtime.motion import Motion, ArrivalEvent, JUMP_TRAVEL_TIME
...
class RealTimeArbiter:
    ...
    def airborne_destinations(self) -> dict[Position, str]:
    ...
    def advance_time(self, ms: int) -> list[ArrivalEvent]:
        ...
        remaining_moves = [m for m in self._motions if self._clock < m.arrival_time]
        airborne_dsts = {m.dst: m.piece_token for m in self._jumps + arrived_jumps}
        self._motions = remaining_moves
