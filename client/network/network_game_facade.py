...
game_over, subscribe_moves/outcomes/game_over) so client/main.py's render loop
...
class MatchmakingError(Exception):
    ...
    lets a blocking caller like client/main.py's Play flow stop waiting and
...
def wait_for_game_start(client, timeout: float = 65.0) -> dict:
    ...
    (e.g. matchmaking timed out with no opponent found).

    timeout is forwarded to every recv_one_blocking() call this makes (not
    just the first) - matchmaking can legitimately take up to
    config.MATCHMAKING_WAIT_MS (60s) before the server gives up, so the
    default comfortably exceeds that rather than using
    WsClient.recv_one_blocking's own 5s default, which would otherwise raise
    an unhandled TimeoutError partway through a normal wait."""
    while True:
        message = client.recv_one_blocking(timeout=timeout)
...
class NetworkGameFacade:
    def __init__(self, client, start: dict, event_logger=None):
        ...
        (see client/tests/unit/test_network_game_facade.py). start: the
        ...
        self._opponent_status_events = Subject()
        self._elapsed_ms = 0
        ...
        self._handlers = {
            ...
            protocol.OPPONENT_DISCONNECTED: self._handle_opponent_disconnected,
            protocol.OPPONENT_RECONNECTED: self._handle_opponent_reconnected,
    ...
    def subscribe_opponent_status(self, callback) -> None:
        """Register callback(event) for OpponentDisconnected/OpponentReconnected
        - drives the disconnect-forfeit countdown banner."""
        self._opponent_status_events.subscribe(callback)
    ...
    def _handle_move_result(self, message: dict) -> None:
        ...
        if message["type"] == protocol.MOVE_ACCEPTED:
            ...
            duration_ms = chebyshev_distance(source, destination) * MOVE_TRAVEL_TIME_PER_CELL
    ...
    def _apply_to_board(self, outcome) -> None:
        ...
        if isinstance(outcome, Arrived):
            ...
            if outcome.by_token is None:
                # Per the engine's own Captured docstring: by_token is None
                # exactly when there's no single capturing piece (a
                # mid-flight/airborne kill) - it never actually left its
                # source cell on the board, so there's nothing to move.
                # (source == position happens to hold in both of today's
                # engine call sites for this case, but by_token is the
                # documented signal - don't key off that coincidence.)
    ...
    def _handle_opponent_disconnected(self, message: dict) -> None:
        self._opponent_status_events.publish(OpponentDisconnected(message["forfeit_in_ms"]))

    def _handle_opponent_reconnected(self, message: dict) -> None:
        self._opponent_status_events.publish(OpponentReconnected())
