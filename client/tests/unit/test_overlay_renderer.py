...
def test_disconnect_countdown_is_drawn_when_given():
    overlays = build()
    canvas = FakeCanvas()
    overlays.draw(canvas, selected=None, halted_positions=[], cooldown_fade_fractions={}, game_over=False,
                   disconnect_countdown_seconds=14)
    assert canvas.texts == [("Opponent disconnected - auto-resign in 14s", 100, 50)]


def test_no_disconnect_countdown_draws_no_banner():
    overlays = build()
    canvas = FakeCanvas()
    overlays.draw(canvas, selected=None, halted_positions=[], cooldown_fade_fractions={}, game_over=False)
    assert canvas.texts == []
